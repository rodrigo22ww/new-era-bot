...
/******** OWNER NUMBER**********/
const ownerNumber = ["+5588997283042@s.whatsapp.net","+5588997283042@s.whatsapp.net"] 
/************************************/
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `akhirnya beban group berkurang 𝟭,flw seu gay 🥳 @${num.split('@')[0]} jasamu akan di kubur dalam²`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
           ...
           case 'demote':
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`SAUVI SAUVI🥳 @${mentioned[0].split('@')[0]} *anda naik menjadi admin group* (+_+)`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
